...
class WhatsAppClient {
    ...
    public function isDeviceOnline() {
        try {
            ...
            // Cek format 8: Jika respons adalah objek dengan properti apa pun
            if (is_object($devices) && !empty((array)$devices)) {
                // Periksa apakah ada properti error atau status yang menunjukkan kesalahan
                if (isset($devices->error) || 
                   (isset($devices->status) && $devices->status === 'error') ||
                   (isset($devices->container['error'])) ||
                   (isset($devices->container['status']) && $devices->container['status'] === 'error')) {
                    error_log('WhatsApp isDeviceOnline: Device offline (error)');
                    return false;
                }
                
                // Jika tidak ada indikasi error, anggap online
                error_log('WhatsApp isDeviceOnline: Device online (format 8)');
                return true;
            }
            
            // Jika tidak ada format yang cocok, anggap offline
            error_log('WhatsApp isDeviceOnline: Device offline (no matching format)');
            return false;
        } catch (\Exception $e) {
